...
| **上下文优化** | 上下文长度处理、质量过滤 | [查看](./context-optimization.md) |
| **上下文工程** | Pipeline 设计、拼接结构、长度控制 | [查看](./context-engineering.md) |
| **RAG评测与优化** | 评测指标、优化策略、效果验证 | [查看](./rag-evaluation.md) |
...
5. **Re-ranker 后如何选择 Top-K？**
   能说明 5-10 这个范围的选择依据，以及如何验证。
6. **上下文过长/过短怎么处理？**
   能说明截断、压缩、扩展等策略的适用场景。
...
6. **Top-K 选择** - Re-ranker 后返回数量、验证方法、动态策略
7. **上下文优化** - 过长截断/压缩、过短扩展、质量过滤
8. **上下文工程** - Pipeline 设计、拼接结构、长度控制策略
9. **实际调优经验** - 如何根据业务选择检索方案
...
5. **RAG评测与优化**：掌握系统评测方法和优化策略，理解如何验证改进效果。

### 复习重点

- 若目标岗位偏 **搜索 / RAG 工程**：优先掌握 BM25、混合检索、父子索引、Re-ranker、上下文优化、上下文工程。
- 若目标岗位偏 **Agent / AI 应用开发**：重点理解混合检索、HyDE、Re-ranker、上下文优化、上下文工程与工具调用质量之间的关系。

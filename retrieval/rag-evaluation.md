...
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐  ┌────────┐ │   │
│  │  │Recall@K │  │Precision│  │   MRR   │  │  NDCG  │ │   │
│  │  │  召回率  │  │  精确率  │  │平均倒数排名│  │归一化折损│ │   │
│  │  └─────────┘  └─────────┘  └─────────┘  └────────┘ │   │
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐             │   │
│  │  │ Hit Rate│  │  MAP    │  │Coverage │             │   │
│  │  │ 命中率  │  │平均精度均值│  │ 覆盖率  │             │   │
│  │  └─────────┘  └─────────┘  └─────────┘             │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
...
│  │  │N-gram匹配│  │召回导向  │  │同义词友好│  │语义相似度│ │   │
│  │  └─────────┘  └─────────┘  └─────────┘  └────────┘ │   │
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐             │   │
│  │  │ GPT-4评分│  │ 人工评估  │  │ 困惑度  │             │   │
│  │  │ LLM-as-judge│  │HumanEval│  │Perplexity│            │   │
│  │  └─────────┘  └─────────┘  └─────────┘             │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
│                           ▼                                 │
...
│  │              忠实度 (Faithfulness)                    │   │
│  │  ┌─────────┐  ┌─────────┐  ┌─────────┐             │   │
│  │  │ FactScore│  │  幻觉检测  │  │ 引用准确性 │             │   │
│  │  │ 事实评分 │  │Hallucination│  │ Attribution │             │   │
│  │  └─────────┘  └─────────┘  └─────────┘             │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
│                           ▼                                 │
...
│                                                             │
│  MRR = (1/|Q|) × Σ(1/rank_i)  其中 rank_i 是第i个查询的      │
│        第一个相关文档的排名                                   │
│                                                             │
│  NDCG@K = DCG@K / IDCG@K                                    │
...
│                                                             │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  查询端优化 (Query Enhancement)                      │   │
│  │  ├── 查询扩展：同义词、相关概念补充                   │   │
│  │  ├── 查询重写：将口语化问题转为结构化查询             │   │
│  │  ├── HyDE：生成假设文档辅助检索                       │   │
│  │  └── 意图识别：区分事实查询/推理查询/比较查询         │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  索引端优化 (Index Optimization)                     │   │
│  │  ├── 分块策略：按语义/段落/固定长度切分               │   │
│  │  ├── 粒度控制：Parent-Child 索引结构                  │   │
│  │  ├── 元数据增强：添加标题、摘要、关键词标签           │   │
│  │  └── 多模态索引：文本 + 表格 + 图片联合索引           │   │
│  └─────────────────────────────────────────────────────┘   │
│                           │                                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │  模型端优化 (Model Optimization)                     │   │
│  │  ├── Embedding 微调：领域适配训练                     │   │
│  │  ├── 混合检索：BM25 + 向量检索融合                    │   │
│  │  ├── Re-ranker：精排模型二次排序                      │   │
│  │  └── 查询分类：不同问题类型路由不同检索策略           │   │
...
│                                                             │
│  【关键原则】                                               │
│  • 检索是上限：检索不到相关文档，生成再好也没用             │
│  • 生成是体验：检索到了但生成质量差，用户体验差             │
│  • 优先优化检索，再优化生成                                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
│                                                             │
│  1. 基线建立                                                │
│     ├── 记录当前系统各指标（Recall@5, Precision@3, BLEU）   │
│     └── 确保测试集稳定不变                                  │
│                                                             │
│  2. 单变量实验                                              │
│     ├── 每次只改一个优化点                                  │

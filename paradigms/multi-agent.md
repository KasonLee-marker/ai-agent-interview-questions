...
```mermaid
flowchart TD
    subgraph Direct["1. 直接通信 Direct\n简单、耦合度高"]
        A1["Agent A"] --> B1["Agent B"]
    end
    subgraph MQ["2. 消息队列 Message Queue\n解耦、异步、可靠"]
        A2["Agent A"] --> Q["Queue"] --> B2["Agent B"]
    end
    subgraph PS["3. 发布订阅 Pub/Sub\n一对多、动态订阅"]
        A3["Agent A"] --> T["Topic"]
        T --> B3["Agent B"]
        T --> C3["Agent C"]
        T --> D3["Agent D"]
    end
    subgraph SS["4. 共享状态 Shared State\n状态同步、最终一致"]
        A4["Agent A"] --> SS1["State Store"]
        B4["Agent B"] --> SS1
        C4["Agent C"] --> SS1
    end
    subgraph BB["5. 黑板系统 Blackboard\n共享工作空间、协作推理"]
        A5["Agent A"] --> BB1["Blackboard"]
        B5["Agent B"] --> BB1
        C5["Agent C"] --> BB1
    end
```
...
```mermaid
flowchart TD
    Start["任务分析"] --> Q1{"任务复杂度是否高？"}
    Q1 -->|No| Single["单体 Agent"]
    Q1 -->|Yes| Q2{"是否存在明确的功能边界？"}
    Q2 -->|No| Enhanced["使用 ReAct 增强单体"]
    Q2 -->|Yes| Q3{"是否需要并行处理？"}
    Q3 -->|No| Pipeline["Pipeline（串行多 Agent）"]
    Q3 -->|Yes| Q4{"各模块是否需要不同能力/模型？"}
    Q4 -->|No| SharedMulti["共享模型的 Multi-Agent"]
    Q4 -->|Yes| SpecialMulti["专用 Agent\n如 Coding 用 GPT-4，简单任务用 GPT-3.5"]
```

...
| 原则 | 说明 | 正例 | 反例 |
|------|------|------|------|
| **面向下一步决策** | 内容以"模型下一步需要什么"为核心，而非原始数据堆砌 | 返回结构化摘要 + 关键字段 | 返回完整 API 响应或原始 HTML |
| **结构化且字段稳定** | 使用 JSON 等可解析格式，字段命名固定，避免随机变化 | `{"status": "success", "data": {...}}` | 每次格式不同的自然语言文本 |
| **信息裁剪** | 对长文本/大数据进行摘要或截断，只保留与当前决策相关的内容 | 搜索结果返回标题 + 摘要（≤200字） | 直接把整个网页内容喂给模型 |
| **错误可恢复** | 错误信息须包含 `error_code`、`retry_after`、`suggestion`，让模型能自主决策下一步 | `{"error_code": "RATE_LIMIT", "retry_after": 60, "suggestion": "稍后重试"}` | 只返回 `"error"` 或 HTTP 500 |
| **可信度与元数据** | 附带 `source`、`timestamp`、`confidence` 等元信息，让模型评估结果可靠性 | `{"data": ..., "source": "weather_api_v2", "timestamp": "...", "confidence": 0.95}` | 只有裸数据，模型不知道数据来源和时效 |
| **安全与脱敏** | 返回前过滤敏感字段（密码、Token、手机号、身份证等），防止信息泄露进入模型上下文 | 用户信息只返回 `username`，隐藏 `phone`/`id_card` | 将用户完整档案直接写入 Observation |

> 💡 **一句话总结：好的 Observation，本质上是给下一轮 Thought 使用的高质量上下文**——它应该是经过裁剪、结构化、标注了来源与可信度的决策依据，而不是原始数据的透传。
...
>
> 工程落地时，核心挑战包括：Prompt 格式稳定性、工具 Observation 设计、防止死循环、以及成本控制。好的 Observation 设计是 ReAct 可靠运行的关键——它应该是**结构化、字段稳定、经过信息裁剪、附带元数据、支持错误恢复且做过脱敏处理**的高质量上下文，而不是原始数据的直接透传。
...
| **Observation 设计** | 给下一轮 Thought 的高质量上下文：结构化 + 裁剪 + 元数据 + 错误可恢复 + 脱敏 |

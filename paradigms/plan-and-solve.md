...
flowchart TD
    subgraph PlanPhase["Plan Phase"]
        Input["任务输入"] --> Analyze["分析需求"]
        Analyze --> MakePlan["制定计划"]
        MakePlan --> Steps["生成步骤列表"]
    end
    subgraph SolvePhase["Solve Phase"]
        S1["Step 1"] --> E1["执行"]
        S2["Step 2"] --> E2["执行"]
        S3["Step 3"] --> E3["执行"]
        E1 & E2 & E3 --> Summary["汇总结果"]
    end
    PlanPhase --> SolvePhase
```
...
flowchart TD
    Error["步骤执行失败"]
    Error --> L1["Level 1: 步骤内重试 Step Retry\n单个步骤失败，重试或换方式执行"]
    L1 -->|重试仍失败| L2["Level 2: 局部重规划 Local Replan\n某步骤失败，调整后续计划"]
    L2 -->|局部重规划仍失败| L3["Level 3: 全局重规划 Global Replan\n整体策略失败，重新制定计划"]
```
